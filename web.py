...
from forms import Login, Mensaje, Registro
...
def send():
    ...
    if request.method == "GET":
      return render_template('send.html' , prueba=frm, titulo='Enviar Mensaje')
    else:
        #cod = request.form["codigo"]
        de = escape(request.form["de"])
        para = escape(request.form["para"])
        asunto = escape(request.form["asunto"])
        mensaje = escape(request.form["mensaje"])
        sql = "INSERT INTO MENSAJES(de, para, asunto, mensaje) VALUEs (?, ?, ?, ?)"
            # Ejecutar la consulta
        #pwd = generate_password_hash(mensaje)
        res = accion(sql,(de, para, asunto, mensaje))
            # Procesar la respuesta
        if res!=0:
                flash('INFO: Mensaje enviado con exito')
        else:
